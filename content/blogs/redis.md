...
这里要注意的是，命令写入AOF缓冲区由主线程完成。而真正刷盘，由后台BIO线程异步处理。所以，AOF的常规追加和everysec刷盘并不会触发COW，因为这个过程不需要fork子进程。
...
在淘汰策略中，LRU和LFU很容易混淆，它们的关注点并不一样。LRU关注“最近一次访问时间”，它认为“最近被访问过的数据，未来大概率还会再次被访问”，因此会优先淘汰“很久没有访问”的数据；LFU关注“访问频率”，它认为“长期访问次数少的数据价值更低”，因此会优先淘汰“累计访问次数最少”的数据。可以简单理解为：
- LRU看“谁最近没被访问”
- LFU看“谁长期不热门”
