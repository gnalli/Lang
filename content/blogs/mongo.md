...
在内存中，每个表都是一颗B+树，只有leaf page存储真实数据。而且数据是未压缩、未加密的原始KV，这保证了访问效率。而在磁盘上，每个表对应一个.wt文件，其内数据组织形式为block。内存中每个page会被写入为一个block。每个block按4KB对齐，并使用extent（offset + length）标识位置。与内存不同的是，磁盘上的数据是压缩后的，可能是加密的。因此，数据从磁盘加载到内存时，需要进行解压与格式转换。
